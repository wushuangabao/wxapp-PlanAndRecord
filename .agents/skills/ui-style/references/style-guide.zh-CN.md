...
| 状态色 | 计划 `#7f8ca1`；候选 `#a58454`；实际 `#55725e`；危险 `#9a5550` |
...
| 语义 | 颜色 | 使用边界 |
| --- | --- | --- |
| 计划 | `#7f8ca1` | 时间轴计划图例、计划状态标记；使用灰蓝色，与灰绿实际记录保持色相差异 |
...
日历计划块按优先级使用以下灰蓝色阶；计划编辑器的三级色点使用对应侧边色：

| 优先级 | 侧边 / 色点 | 背景 | 文字 |
| --- | --- | --- | --- |
| 低 | `#a7afbd` | `#eff1f4` | `#596577` |
| 中 | `#7f8ca1` | `#dde3ea` | `#47566b` |
| 高 | `#68788f` | `#c7ced8` | `#3f4d60` |
