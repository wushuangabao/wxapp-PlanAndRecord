...
`Wish / 备忘录 → 项目 / 任务 / OKR → 日历计划 → 计时或补录 → 候选核实 → 统计复盘 → 主动导出`

MVP 完成时，用户应能在无登录、无后端、无业务网络请求的情况下：
...
- 主动导出 JSON 全量数据和 CSV 日志，并理解首版不能导入或云端恢复。
...
| 设置与导出 | 分类管理、本地数据风险提示、JSON 全量导出、CSV 日志导出 |

### 2.2 明确不做

- 微信登录、云开发、自建后端、云数据库、同步、跨设备一致性和云端恢复。
- OCR / LLM、照片或文件智能导入、VIP、广告、支付、订阅消息和云端提醒。
- JSON 导入恢复、团队协作、原生移动端、手表或 AI 眼镜能力。
...
当前仓库只有设计文档和 `project.config.json`，尚无可编译源码、页面或测试设施，因此按全新工程实施。
...
| M5 统计、设置与导出 | 汇总、偏差、重叠提示、周复盘、JSON/CSV 导出和风险提示 | 统计口径由固定数据集验证；导出完整且不宣称可恢复 |
...
| 仓储集成测试 | 原子写入、迁移快照、损坏/高版本保护、导出一致性 |
| 页面回归 | 四页签主流程、候选核实、确认框、空态和失败反馈 |
| 模拟器与真机 | 后台/前台、杀进程冷启动、跨日期、文件导出 |
| 隐私与范围审查 | 无业务网络请求、无云/登录入口、日志不泄露完整业务数据 |
...
8. 导出 JSON 和 CSV，确认版本、关系、状态、来源及本地数据风险提示。
...
- 用户页和导出入口均明确提示本地数据可能丢失，JSON 不可导入恢复。
...
- M1：OKR、重复规则、优先级等尚未固化字段的 Schema。
- M2：设备系统时间被修改时，现有恢复规则未覆盖的边界行为。
- M4：首版重复频率集合、日历交互与统计展示草图。
- M5：周起始日、CSV 列与编码、文件命名、临时文件清理及导出失败出口。
