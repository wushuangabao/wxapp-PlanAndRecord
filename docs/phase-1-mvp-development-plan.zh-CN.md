...
- 在日、周、月、年日历查看数据上相互独立的计划、候选和实际记录；重复规则虚拟实例已有日志时，画布以关联日志替代该虚拟计划块，统计仍保留计划时长与偏差。
- 创建固定日程，并确认、跳过或删除本次及后续的按需投影重复实例；完整行为见[产品设计基线 5.3 节](./product-design.zh-CN.md#53-计时器与重复规则)。
...
| 重复规则 | 固定日程只持久化含单一 revision 的 `RepeatRule`、`virtual plan` 的确定性按需投影、仅 `skip` 例外，以及经二次确认的“删除本次及后续”；完整契约见[产品设计基线 5.3 节](./product-design.zh-CN.md#53-计时器与重复规则) |
...
- 网络查询中国法定放假或调休安排；首版日历脚标只用内置表，异步查询见[产品设计基线 4.3.2 节](./product-design.zh-CN.md#432-节假日安排的异步查询)。
...
- `CalendarEvent` 只表示普通计划；固定日程只持久化 `RepeatRule`，包含首项在内的实例全部按需投影且不创建种子 `CalendarEvent`；`TimeLog` 只表示 `candidate` 或 `confirmed`。
...
| M4 日历与重复规则 | 四种日历视图、统一时间轴、计划关联、固定日程仅保存含单一 revision 的 `RepeatRule`、`virtual plan`、仅 `skip` 例外、删除本次及后续和日历逐卡重叠标记 | 创建固定日程不写入 `CalendarEvent`；多次查询不重复；计划、候选、实际不互改底层数据；删除须二次确认，边界后日志保留并解除计划关联、草稿清空完整 origin pair；重复规则虚拟实例已有日志时按产品基线折叠显示，统计保留计划 |
...
4. 创建重复日程，验证只新增含单一 revision 的 `RepeatRule` 而不新增 `CalendarEvent`，并验证首项及后续 `virtual plan` 投影、跳过、删除本次及后续的二次确认、截止边界、日志保留解绑与草稿清空完整 origin pair，以及跨页面去重；JSON 只接受单一 revision 和 `skip` 例外。完整规则见[产品设计基线 5.3 节](./product-design.zh-CN.md#53-计时器与重复规则)。
