...
> 编制日期：2026-07-28
> 最近同步：2026-08-04（2.25.4 / 3.16.2 双目标模拟器验收已完成；真机与危险写路径待完成）
...
| M5 统计、设置与 JSON 数据管理 | 标签/项目汇总、派生“无标签”桶、偏差、周复盘、JSON 导出、增量/覆盖导入、清空和风险提示 | 统计口径由固定数据集验证；JSON 手动迁移/恢复闭环完整且不宣称云端备份或同步 |
...
计划执行中产生的页面级任务、完整测试矩阵、字段定义和未来结构升级脚本设计，在对应里程碑启动时单独补充，不纳入本 MVP 总计划；本次旧投入维度收敛为 `TimeLog.tags` 不编写兼容迁移脚本。延期项仅见 [document-review-backlog.zh-CN.md](./document-review-backlog.zh-CN.md)，不写入当前里程碑。
