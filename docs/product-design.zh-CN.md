...
- 不自动确认用户实际做过某事。日历规则、OCR 和文本 AI 解析只产出候选；自动恢复的异常计时按 [5.3.1 超时恢复审核流程](#531-超时恢复审核流程) 只创建待审核的恢复草稿。用户主动点击“结束计时”创建记录，或手工修正恢复草稿后保存记录时，视为完成确认。JSON 全量快照导入只恢复快照中已有的状态，不把 `candidate` 自动转为 `confirmed`。
...
| 项目 | 有明确边界和截止日期、OKR 可后补的承诺 | 直接关联 OKR 和任务；计划块与日志的项目维度只通过任务派生 |
...
| 状态 | 含义 | 默认计入“实际投入” |
| --- | --- | --- |
| `candidate` | 来自重复日程、OCR 或文本 AI 导入，等待用户核实 | 否 |
...
时间日志的结束时间不得早于开始时间；两者相同时可保存为即时记录，并按短时计时口径将 `durationMinutes` 记为 `1`。计划块和重复规则仍要求结束时间晚于开始时间。
...
- 新建项目只要求项目名称与有效截止日期，`Project.objectives` 可以为空；目标和关键结果均可在后续补充。任何已保存的目标必须有名称；关键结果必须隶属于目标，且具有名称与 `[0, 100]` 的整数完成情况。
...
- 项目视图：一个纵向列表，位于 TODO LIST 下方，展示所有项目（有截止日期、可按需补充 OKR 的复杂任务）。
  - 活动项目列表受 `MAX_ACTIVE_PROJECTS = 5` 限制；已有等于上限的活动项目时，新增和 Wish 转换都不得创建项目，并提示用户先归档、放弃或取消本次操作。
  - 列表末尾右对齐的“+”打开新建项目底部弹层；达到上限时隐藏该入口。新建项目只要求标题与截止日期；OKR 可留空并在后续补充，字段约束见 [3.3 其他核心对象边界](#33-其他核心对象边界)。
...
- 添加愿望时只需输入不超过 25 个字符的描述；添加项目时需要输入不超过 25 个字符的描述和截止日期，OKR 可后补。
