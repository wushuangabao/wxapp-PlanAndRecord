...
| 任务 / TODO | 可完成的单个行动项 | 可关联项目，也可直接排入日历或启动计时 |
...
- 显式删除单个任务需要二次确认。删除任务实体时，删除其关联且结束时间晚于删除时刻的日历事件，以及这些事件/任务关联的重复规则、修订和例外；保留已结束日历事件、全部时间日志和计时草稿。保留的实体清空失效的 `taskId`、`calendarEventId` 或 `originRuleId` 并保留相应名称快照；删除任务不删除其他实体，也不清除它们仍有效的项目关联。
...
- 可修复的失效关联不视为文件损坏。引用是否失效必须以增量补充和冲突决策完成后的最终合并结果判断；只要被引用实体存在于最终结果中，就不得因它未出现在导入文件内而清空引用。导入预览必须显示“修复了 N 个失效关联”，`N` 按被清空、重映射或丢弃的失效关联逐项计数，修复规则为：
  - 缺失的项目、任务、计划块、重复规则等可选引用清为 `null`，并保留已有的项目、任务、计划块或重复规则名称快照。
...
- JSON 结构错误、必需字段或字段类型错误、导入文件内部出现重复 ID、`schemaVersion` 缺失或版本不受支持等属于不可修复的数据损坏。发现任一此类问题即拒绝整次导入，不生成可提交的合并结果，也不修改任何本地数据。
...
- 清空前必须用明确的破坏性提示二次确认；用户取消时不修改数据。清空必须原子写入，失败时保留原数据并提示失败，不得显示成功。
