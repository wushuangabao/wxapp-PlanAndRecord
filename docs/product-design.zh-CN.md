...
- **低摩擦记录，延后精化。** 开始时只需最少操作；分类、计划块关联、备注和标签可随时补充和修改。项目归属不在日志上单独维护，而由计划块关联的任务派生。
...
| 时间日志 | 一个时间区间内实际或待核实的活动 | 直接关联分类，可选关联一个计划块，并保留备注、来源与修订；不得直接关联任务或项目 |
...
计划与实际偏差以单个计划块实例为比较单位：非重复计划块是一个 `CalendarEvent`，重复计划块是重复规则投影出的一个虚拟实例。一个计划块实例可以关联零到多条 `TimeLog`；一条 `TimeLog` 最多关联一个计划块实例，关联可空，且不能只因分类、任务或项目相同就自动建立。
...
- 未关联计划块的日志仍计入分类和总实际投入，在偏差统计中单列为“非计划实际”，不归入任何计划块，也不具有可统计的任务或项目归属。
...
新建日志只能选择 `active` 分类。编辑既有日志时，如果没有更换分类，即使原分类已经归档，也必须原样保留其 `categoryId` 与名称快照；只有新选或改选分类时才要求目标分类为 `active`。既有日志若关联了一个因任务删除而变为只读历史的计划块，编辑其他字段时同样保留该关联；一旦解除关联，不得重新选择该计划块。

允许时间重叠，但在保存时给出提醒；例如用户可能希望保留“通勤时听课程”这种双重归类。分类、项目、总投入、容量和预测等时长汇总均累加每条纳入统计的日志 `durationMinutes`，不按墙钟区间去重，因此汇总时长可以大于真实经过的墙钟时长。其中项目统计必须先由日志找到计划块，再由计划块的任务找到 `Task.projectId`；链路任一环缺失时不计入具体项目。
...
- `Category` 至少包含稳定 `id`、名称和 `active / archived` 状态。每个本地资料库都有不可删除的“未分类”；用户没有选择分类时使用它。已被日志引用的其他分类只能归档，归档后不再用于新记录或改选目标；只编辑日志的其他字段时不得要求原分类重新变为 `active`，日志继续保留原 `categoryId` 与 `categoryNameSnapshot`。
...
- 首页提供“手工补录”次级入口，可填写任意过去时间区间、分类及可选计划块；不选计划块时记为“非计划实际”，用户主动保存的手工记录直接生成 `confirmed` 日志。
- 用户可随时修改当前记录的分类、可选计划块、标签和备注；计时与补录界面不提供任务或项目选择控件。选择计划块后，任务由计划块确定，项目再由任务的 `projectId` 派生；生成记录后保存数据并清空表单。
...
4. AI 将 OCR 结果解析为导入草稿：开始时间、结束时间、事项、分类、可选计划块匹配建议与备注等；不得直接生成日志的任务或项目关系。
...
第二阶段的文本 AI 导入直接把用户提交的文本解析为导入草稿，无需 OCR；用户确认后创建的日志保留 `source = "file"`。
...
- 用户可在创建或编辑日志时选择一个计划块；具体 `CalendarEvent` 使用 `calendarEventId`，重复规则投影实例使用 `originRuleId + originOccurrenceId`。从计划块启动计时时预填该关联；不选时为“非计划实际”。日志关系控件只提供计划块和分类，不提供任务或项目。
...
- 分类汇总直接使用日志的分类关系；项目汇总必须由日志关联的具体计划块或重复投影实例取得任务，再读取该任务当前有效的 `projectId`。无计划块、计划块缺少有效任务或任务未关联项目的日志不归入具体项目；任务/项目名称快照只用于历史展示，不参与当前项目统计。
...
- 首版唯一有效的业务关系链是 `TimeLog → 计划块实例 → Task → Project?`：日志可不关联计划块；计划块必须关联任务；任务可不关联项目。具体计划块通过 `calendarEventId` 关联，重复投影实例通过 `originRuleId + originOccurrenceId` 关联。分类始终是 `TimeLog → Category` 的独立直接关系。
...
运行中的计时器至少持久化开始时间、当前状态、暂停区间或累计暂停时长，以及当前填写的分类、可选计划块关联、备注和标签；不得持久化可编辑的任务或项目关系。
...
- JSON 全量导出包含实体 ID、关系、`schemaVersion`、日志状态与来源。接收文件名为 `plan-and-record-YYYYMMDD-HHmmss.json`，使用 UTF-8 编码。
...
- 用户选择以下一种导入模式：
  - **增量导入**：保留本地数据，加入本地尚不存在的实体。同一 ID 且内容相同的实体直接跳过；同一 ID 但内容不同的实体计为数据冲突。内容比较基于解析后的全部持久化字段，包含 `createdAt`、`updatedAt` 等元数据并保留数组顺序，但忽略 JSON 对象属性的书写顺序，禁止直接比较原始 JSON 字符串。
...
- 可修复的失效关联不视为文件损坏。引用是否失效必须以增量补充和冲突决策完成后的最终合并结果判断；只要被引用实体存在于最终结果中，就不得因它未出现在导入文件内而清空引用。导入预览必须显示“修复了 N 个失效关联”，`N` 按被清空、重映射或丢弃的失效关联逐项计数，修复规则为：
  ...
  - 日志引用的分类不存在时，映射到系统“未分类”，并使用该分类的名称快照。
...
- JSON 结构错误、必需字段或字段类型错误、导入文件内部出现重复 ID、同一日志或计时草稿同时声明具体计划块与重复计划关联、`originRuleId` 缺少配套 `originOccurrenceId`、`schemaVersion` 缺失或版本不受支持等属于不可修复的数据损坏。历史日志单独保留 `originOccurrenceId` 的情况按上文来源追溯例外处理。发现任一不可修复问题即拒绝整次导入，不生成可提交的合并结果，也不修改任何本地数据。
...
- “清空数据”删除当前匿名资料库的全部用户数据、计时状态、恢复草稿、结构迁移前备份，以及小程序文件沙箱内由本产品明确生成且仍然存在的导出临时文件；随后重新建立一个空资料库及系统“未分类”。已由用户发送或另存到微信会话、电脑等外部位置的 JSON 文件不属于小程序本地数据，不受清空操作影响。
