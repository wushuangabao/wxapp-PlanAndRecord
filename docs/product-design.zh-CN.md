...
- TODO LIST：位于页眉之后、项目列表之前。每 3 条可见任务横向分列；禁止纵向滚动，横向滑动按整列吸附，且不显示行分隔。普通任务展示勾选框；有关联计划的任务改用颜色可区分的计时器 icon 表达执行入口。任务标题仍显示可选项目名称小字，并保留“关联项目”和“删除”图标按钮。计划执行状态统一遵守 [3.1.2 任务、计划实例与完成状态](#312-任务计划实例与完成状态)。
  ...
  - 计时器 icon 支持微信默认长按，松手判定与标题长按相同：落点仍在 icon 内才提交，移出则取消。长按后按与计时入口相同的候选口径处理。没有可确认候选时不写入，并按当前 icon 状态提示“今天的固定日程已记录”“今天没有这项固定日程”或“当前没有可确认的计划”。只有一条候选时，若该计划正在计时则提示“该计划已经在计时了”且不弹出确认框，否则先弹出「直接完成？」确认框；确认后立即写入 `confirmed` 日志，取消不写入。有多条时先用与点按计时相同的计划选择层让用户明确选择，再按同一规则检查计时并弹出确认框。虚拟实例的写入规则、来源和“正在计时不可确认”约束与日历「确认完成」相同；普通计划块按该 `CalendarEvent` 的起止时间生成 `source = manual` 的 `confirmed` 日志并关联 `calendarEventId`，若当前活动计时已关联该计划块则同样拒绝并提示先结束计时。长按不改变点按开始计时。该能力同时适用于顶部 TODO LIST 与项目卡片中的计时器 icon。
...
- 普通实体计划详情按该 `CalendarEvent` 当前有效关联的 `confirmed TimeLog` 派生记录上下文。尚无已确认记录时，主操作仍显示「开始计时」，并按既有条件提供「直接完成」；已有一条或多条已确认记录时，主操作改为「再次开始计时」，继续隐藏「直接完成」，但不限制后续计时继续为同一计划生成新的实际记录。已有记录时，详情基本信息与操作区之间显示「已有记录（N）」列表，仅纳入通过 `calendarEventId` 关联的 `confirmed` 日志，不纳入 `candidate`、标题相同或旧兼容 `TimeLog.taskId` 推断结果；按实际开始时间由早到晚排列，每行使用统一日志标题回退规则，并显示实际起止时间与时长。记录较多时只让列表区域纵向滚动，弹窗头部和底部操作保持可访问。记录行支持微信默认长按：长按阶段只暂存目标，松手仍落在原记录行内时才关闭详情并定位，手指移出该行或触摸取消时不执行；矩形捕获与松手判定必须复用项目现有长按公共逻辑。定位保持当前日、周、月或年视图类型，把锚点切到记录开始时间所在的当前粒度范围并滚动到目标记录块；当前筛选为「只看计划」时切为「只看记录」，「查看全部」或「只看记录」保持不变。目标记录复用既有「+N」保护与持续 3 秒的灰绿描边反馈。该展示、按钮文案与定位只读取现有关系，不修改计划、日志、任务状态或统计口径。
- 重复实例的详情提供“编辑、开始计时、确认完成、跳过、删除本次及后续”。“编辑”固定表示编辑“本次及后续”，不提供“仅此一次”，也不追溯修改过去实例；若所选实例及后续已有任意 `candidate` / `confirmed` 时间日志、活动计时草稿或恢复草稿关联，则隐藏编辑按钮，应用服务保存时仍须在事务内重新校验并拒绝写入。若当前活动计时（运行中或已暂停）已关联该实例，则隐藏“确认完成”，且不得在计时结束前把该实例直接确认成记录。删除操作先显示二次确认：标题为“删除本次及后续”，内容为“将删除本次及之后的固定日程。已有时间记录会保留，但会解除计划关联。”，确认按钮为“删除”；确认成功后提示“本次及后续已删除”，关闭详情并刷新当前范围。取消、校验失败或存储写入失败时不改变页面数据。
...
- 左右滑动画布切换相邻时间范围时，旧范围沿滑动方向退出、新范围从相反方向进入，以短促翻页过渡明确反馈范围已经变化；翻页提交后在画布顶部居中悬浮显示目标范围相对设备今天的关系，日视图使用“昨天 / 今天 / 明天”“前天 / 后天”及更远的“N 天前 / N 天后”，周、月、年视图对应使用“上 / 本 / 下”和更远的数字差值。从计时、计划、用户或数据恢复等任意其他页面切换到日历时也显示同一提示：普通切换使用日历保留的当前粒度和锚点，携带计划或记录定位 Handoff 时等待目标粒度、锚点和画布刷新完成后再显示；应用冷启动时日历页的首次显示、日历页自身重复显示或应用从后台恢复但可见页面未改变时不提示。提示使用与记录页“最近记录”中 `new` 一致的 `#9a5550` 粗体大字，持续 3 秒后消失；期间再次翻页或重新进入日历时更新文案并重新计时。过渡与提示均不修改日期切换规则或数据。
...
- 未操作实例只存在于视图投影中，即 `virtual plan`，不是 `candidate` 日志。当前活动计时已关联该实例时，不得再确认完成该实例；应先结束计时。计划页计时器 icon 的长按确认完成与日历「确认完成」共用同一写入规则；尚未记录的普通计划块则按计划起止时间生成 `manual` 来源的 `confirmed` 日志。
