...
- 统计查询先分别按日志和计划块实例是否与查询范围存在非零交集确定纳入项。若一条已纳入日志具有有效计划块关联，但其关联计划块实例本身尚未与查询范围相交，则把该计划块实例随日志一并纳入同一次统计；同一计划块实例被多条日志带入时只计一次。被带入的实例同时进入计划数、计划时间、偏差明细和对应偏差趋势桶，其计划时长仍取完整实例时长。没有纳入日志关联的未来或范围外计划不得因此提前进入统计。该规则同样适用于普通 `CalendarEvent` 与重复规则投影实例，保证同一查询范围满足 `总实际投入 - 总计划时间 = 偏差合计 + 计划外投入`。
- 计划时长等于计划块实例的结束时间减开始时间；实际时长等于关联日志的时长之和；偏差等于实际时长减计划时长。实际为零表示计划尚未产生关联的实际记录，实际超出计划时显示为超时。
- 未关联计划块的日志仍计入标签汇总和总实际投入；没有标签时计入派生的“无标签”统计桶。这些日志汇总为独立的“计划外”投入分钟数，不归入任何计划块，也不进入以计划块实例为单位的偏差趋势或偏差明细；“我的”页面按第 4.4.1 节将该合计作为总投入的辅助说明展示。
...
- 实体计划不因 `endedAt` 已经过期而失去执行资格；只要 `CalendarEvent` 仍存在并有效关联任务，用户就可在计划开始前提前开始计时，也可晚于计划时间从该计划开始计时。开始计时与不经计时的直接确认使用不同资格：普通计划与固定日程实例只有在设备当前时间到达其 `startedAt` 后，才可通过计划页计时器 icon 长按或日历详情的「直接完成 / 确认完成」生成记录；开始时间之前页面不提供这些确认入口，应用服务在事务内按本次操作时间重新校验并拒绝绕过。仅当任务关联的每个 `CalendarEvent` 都至少有一条 `confirmed` 日志，且任务不再关联任何仍有效的 `RepeatRule` 时，任务才自动转为 `completed` 并写入 `completedAt`。
...
- 编辑普通计划时允许开启“固定日程”。保存时以同一份表单值更新标题、开始/结束时间、任务和优先级，并原子创建对应的单 revision `RepeatRule`。应用服务在事务内判断原 `CalendarEvent` 是否被任意 `TimeLog`（无论 `candidate` 或 `confirmed`）、活动计时草稿或恢复草稿引用：完全无引用时删除原 `CalendarEvent`，使其转换为仅由规则投影的虚拟计划；存在任一引用时保留并更新原 `CalendarEvent`，同时新增与其同名、同时间、同任务、同优先级的固定日程。两条路径都不迁移或解除既有记录与计时关联，且界面统一反馈“固定日程已创建”，不要求用户理解内部替换或并存差异。创建规则、按条件删除或更新普通计划，以及受影响任务状态重算必须在同一事务中完成；任一步失败时不得留下半成品规则或丢失原计划。
- 首版重复规则支持每日、每周（指定星期）和每月（指定日期）。每条 `RepeatRule` 只持久化一个 revision，`revisions.length === 1`；不同时创建首项 `CalendarEvent`，包含首项在内的全部实例均由唯一 revision 按需投影。界面使用非负整数“间隔数”：`0` 表示每个周期都发生，`n` 表示相邻两次发生之间空出 `n` 个周期；页面提交时将其换算为持久化的正整数步长 `RepeatRevision.interval = 间隔数 + 1`。间隔数与持久化步长都必须是 JavaScript 安全整数，且加一后的步长仍须处于安全整数范围内；输入包含符号、小数点、字母或其他非数字字符时不得删除字符后静默改写为另一个周期。revision 的 `effectiveFrom` 是首次可投影的逻辑槽位，`effectiveUntil = null` 表示未结束；“删除本次及后续”以 `occurrenceStart - 1` 设置其截止边界。编辑固定日程同样只支持“本次及后续”：旧规则保留历史范围并在边界前截止，未来部分由新 `ruleId`、单 revision 规则承接，不引入多 revision 或 override，完整安全规则见 [5.3 本地计时与重复规则投影](#53-本地计时与重复规则投影)。产品尚未发布，首版契约从未包含固定日程种子计划块、多 revision 或 override，不提供相关旧结构识别、兼容或迁移，继续使用 `schemaVersion = 1`。
...
- “最近记录”的记录标题按下时显示浅灰绿背景；微信默认长按只暂存当前记录，松手仍落在原标题内时才切换到日历日视图并定位对应时间记录块，手指移出标题或触摸取消时不执行。定位使用一次性会话参数，不写入资料库；日历筛选、滚动、“+N”保护和 3 秒灰绿描边与其他记录定位入口保持一致。
...
- TODO LIST：位于页眉之后、项目列表之前。每 3 条可见任务横向分列；禁止纵向滚动，横向滑动按整列吸附，且不显示行分隔。普通任务展示勾选框；有关联计划的任务改用颜色可区分的计时器 icon 表达执行入口。任务标题仍显示可选项目名称小字，并保留“关联项目”和“删除”图标按钮。计划执行状态统一遵守 [3.1.2 任务、计划实例与完成状态](#312-任务计划实例与完成状态)。
  ...
  - 计时器 icon 支持微信默认长按，松手判定与标题长按相同：落点仍在 icon 内才提交，移出则取消。长按只处理已到达 `startedAt` 的可确认候选；点按计时仍使用全部可启动候选，不受此限制。没有可确认候选时不写入，并按当前 icon 状态提示“今天的固定日程已记录”“今天没有这项固定日程”或“当前没有可确认的计划”。只有一条可确认候选时，若该计划正在计时则提示“该计划已经在计时了”且不弹出确认框，否则先弹出「直接完成？」确认框；确认后立即写入 `confirmed` 日志，取消不写入。有多条时先用与点按计时相同样式的计划选择层列出可确认候选，让用户明确选择，再按同一规则检查计时并弹出确认框。虚拟实例的写入规则、来源和“正在计时不可确认”约束与日历「确认完成」相同；普通计划块按该 `CalendarEvent` 的起止时间生成 `source = manual` 的 `confirmed` 日志并关联 `calendarEventId`，若当前活动计时已关联该计划块则同样拒绝并提示先结束计时。长按不改变点按开始计时。该能力同时适用于顶部 TODO LIST 与项目卡片中的计时器 icon；开始时间门槛与事务校验以 [3.1.2](#312-任务计划实例与完成状态) 为准。
  ...
  - 顶部 TODO LIST 与项目卡片的任务标题支持微信默认长按，不另计 2 秒。长按后优先按与计时入口相同的候选口径处理尚未记录的实体计划，以及今天尚未记录的固定日程虚拟实例：只有一条时直接跳到该计划，不弹出列表；有多条时打开「查看计划」底部弹窗，样式与日历「选择任务」一致，固定头部与取消，仅选项列表滚动，每行显示计划标题与起止时间，点选一项后关闭弹窗并跳转，点遮罩或取消只关闭。没有计划候选时，再查找通过当前有效关系链 `confirmed TimeLog → CalendarEvent / RepeatRule 实例 → Task` 关联到该任务的时间记录；不得使用 `taskNameSnapshot`、标题相同或旧兼容 `TimeLog.taskId` 猜测关系，`candidate` 日志也不进入此入口。只有一条关联记录时直接跳到该记录，不弹出列表；有多条时打开同样结构的「查看记录」底部弹窗，每行按日历记录块的标题回退规则显示名称与实际起止时间，点选后跳转。既没有计划候选也没有关联记录时，未完成任务才询问「创建计划？」；已完成任务不执行任何操作，不得用创建计划入口改变其已完成语义。未完成任务确认创建后留在计划页并打开复用的「新增计划」弹窗，标题预填任务标题，隐藏任务选择，提交普通计划或固定日程时直接关联该既有任务，不得再创建同名 TODO；取消仍留在计划页。计划或记录的跳转都使用 tab 页一次性会话参数，不写入资料库。目标为已有计划时：日历切到日视图，锚点改到该计划开始日；仅当当前筛选为「只看记录」时改为「只看计划」，其他筛选保持。目标为时间记录时：日历切到日视图，锚点改到该记录开始日；仅当当前筛选为「只看计划」时改为「只看记录」，其他筛选保持。两类目标的定位、滚动与「+N」保护都复用新建成功后的规则；目标块显示 `4rpx`、`#78947f` 描边，持续 3 秒后消失，切换粒度、翻页或离开页面也立即清除。长按只导航，不修改任务、计划或记录，也不改变点按编辑标题。
  - 「新建 TODO」弹窗右下提供「设定时间」：在计划页内打开与日历复用的新增计划表单；隐藏任务选择，保留固定日程；提交时按「新建同名任务」在同一事务内创建普通计划或固定日程。从项目下新建 TODO 进入时，同名新建任务关联该活动项目；从独立 TODO 入口进入时不关联项目。`taskProjectId` 不得指向归档或不存在的项目。提交标题以计划表单当前值为准（新任务与计划同名）；取消或点遮罩返回「新建 TODO」并恢复进入「设定时间」前的任务名称。创建成功后留在计划页并刷新列表，不跳转日历、不做画布定位。日历页仍不得提供直接关联项目的控件或写入路径；计划块 `projectId` 保持为空，项目归属只由任务推导。
...
- 项目视图：一个纵向列表，位于 TODO LIST 下方，展示所有项目。
  ...
  - 每个活动项目默认按保存顺序展示前三条未完成子任务；更多未完成项和已完成项可分别在当前项目内展开，展开状态仅保留在当前页面会话中。子任务左侧操作按其派生的任务计划状态执行普通勾选、计划计时、长按确认完成或取消自动完成，点击标题原地编辑（已关联计划时的标题确认、计时器 icon 长按确认完成均与顶部 TODO LIST 相同）；“+ 子任务”会预选当前项目，无未完成子任务时显示“还没有子任务”；仅在项目没有任何子任务时显示“+ 添加第一项子任务”，已有已完成子任务时显示“+ 添加子任务”。“…”收纳编辑、归档与放弃；不显示项目完成率、进度条、百分比或其他项目进度聚合。
...
- 普通实体计划详情按该 `CalendarEvent` 当前有效关联的 `confirmed TimeLog` 派生记录上下文。尚无已确认记录时，主操作仍显示「开始计时」；设备当前时间到达计划 `startedAt` 后再按既有条件提供「直接完成」，开始时间之前隐藏该入口。已有一条或多条已确认记录时，主操作改为「再次开始计时」，继续隐藏「直接完成」，但不限制后续计时继续为同一计划生成新的实际记录。已有记录时，详情基本信息与操作区之间显示「已有记录（N）」列表，仅纳入通过 `calendarEventId` 关联的 `confirmed` 日志，不纳入 `candidate`、标题相同或旧兼容 `TimeLog.taskId` 推断结果；按实际开始时间由早到晚排列，每行使用统一日志标题回退规则，并显示实际起止时间与时长。记录较多时只让列表区域纵向滚动，弹窗头部和底部操作保持可访问。记录行支持微信默认长按：长按阶段只暂存目标，松手仍落在原记录行内时才关闭详情并定位，手指移出该行或触摸取消时不执行；矩形捕获与松手判定必须复用项目现有长按公共逻辑。定位保持当前日、周、月或年视图类型，把锚点切到记录开始时间所在的当前粒度范围并滚动到目标记录块；当前筛选为「只看计划」时切为「只看记录」，「查看全部」或「只看记录」保持不变。目标记录复用既有「+N」保护与持续 3 秒的灰绿描边反馈。该展示、按钮文案与定位只读取现有关系，不修改计划、日志、任务状态或统计口径；直接确认的统一资格与事务校验以 [3.1.2](#312-任务计划实例与完成状态) 为准。
- 重复实例的详情提供“编辑、开始计时、确认完成、跳过、删除本次及后续”。“编辑”固定表示编辑“本次及后续”，不提供“仅此一次”，也不追溯修改过去实例；若所选实例及后续已有任意 `candidate` / `confirmed` 时间日志、活动计时草稿或恢复草稿关联，则隐藏编辑按钮，应用服务保存时仍须在事务内重新校验并拒绝写入。设备当前时间到达实例 `startedAt` 后才提供“确认完成”；开始时间之前，或当前活动计时（运行中或已暂停）已关联该实例时，隐藏该入口，且不得把该实例直接确认成记录。删除操作先显示二次确认：标题为“删除本次及后续”，内容为“将删除本次及之后的固定日程。已有时间记录会保留，但会解除计划关联。”，确认按钮为“删除”；确认成功后提示“本次及后续已删除”，关闭详情并刷新当前范围。取消、校验失败或存储写入失败时不改变页面数据。直接确认的统一资格与事务校验以 [3.1.2](#312-任务计划实例与完成状态) 为准。
- 日历首次进入默认展示日视图。顶部左侧显示当前浏览时间，右侧依次提供“今天、年、月、周、日”；粒度按钮等宽 `54rpx`。标题与右侧控制项按首行垂直对齐。日视图显示该日农历日期（含闰月，形如“农历七月初八”）。该日若逢传统节日或二十四节气，在农历月日后以“·”接上短名，例如“农历七月十一·处暑”“农历八月十五·中秋”；同日两者都有且名称不同时按“节日·节气”依次接上，名称相同则只显示一次。传统节日按非闰农历月日识别，含春节、元宵、端午、七夕、中秋、重阳、腊八和除夕；节气按所显示的公历自然日、以东八区日界判定。范围内有条目时，农历占用次行左侧并与颜色图例对齐；范围内无条目时，空范围提示仍占用次行左侧并与图例对齐，农历上移到范围标题正下方并以更小字号显示。周、月、年视图次行左侧仍显示空范围提示。农历日期只用于展示，由客户端按设备本地自然日换算，不写入资料库，不进入 JSON 导入导出，也不改变计划投影、筛选、统计或容量。空范围提示与颜色图例按次行垂直对齐。当前浏览范围包含设备当天时，“今天”显示 `4rpx` 描边，否则不显示描边。点击“今天”后先将锚点重置到设备当前时刻，待当前粒度画布刷新完成后，再自动把当前时间所在行滚动到可见区域：年定位当月、月定位当天日期、周定位当天、日定位当前小时前一小时以保留上下文（`00:00` 时定位首行）。年、月、周、日互相切换时保留当前锚点日期；若切换前的浏览范围包含设备当天，且新粒度范围仍包含当天，待画布刷新完成后按与点击“今天”相同的规则把当前时间线滚动到可见区域。切换前范围不含当天，或新粒度范围不再包含当天时，恢复该粒度上一次的纵向浏览位置；该粒度首次进入时停留在顶部，日视图且范围包含当前时间时可定位至当前小时。年视图长按左侧某个月份文案并松手后，把锚点改到该月并进入月视图；月、周视图长按左侧某一天文案并松手后，把锚点改到该日并进入日视图。该手势不改变单击折叠、顶部粒度按钮或左右翻页规则；日视图左侧刻度不提供下钻。进入目标粒度后的纵向定位仍遵守上述粒度切换滚动规则。粒度切换按钮下方保留右对齐的“计划 / 记录 / 候选”颜色图例，其中“记录”对应 `confirmed` 日志，图例颜色与画布条目一致。图例右侧提供同排筛选按钮，其右边缘与首行控制项右边缘对齐；首次进入显示“查看全部”，连续点击依次切换为“只看计划”“只看记录”“查看全部”。“只看计划”仅保留普通计划和重复规则虚拟计划，“只看记录”同时保留 `confirmed` 与 `candidate` 日志；筛选只改变当前日历画布展示，不修改实体、关联、统计或当前浏览范围，并在年、月、周、日粒度切换及相邻范围翻页时保持当前筛选状态。周固定为周一至周日，跨月或跨年时标题必须保留足以消除歧义的月份和年份。
- 周、月、年视图中，某个时间格的流式内容超过折叠后仍保留的行数时，左侧时间文案成为折叠开关并显示展开状态指示；默认展开。月、年视图折叠后只保留第一行；周视图因单位格更高，折叠后仍保留前两行。再次点击恢复全部内容。折叠后可见行数不会减少的时间格不提供折叠操作，日视图保持原有分轨规则。年、月、周视图左侧文案的长按下钻与单击折叠互不抢占：单击仍只切换折叠，长按才进入更细粒度。折叠只改变当前页面会话的展示，不删除条目或修改业务数据。
...
- 左右滑动画布切换相邻时间范围时，旧范围沿滑动方向退出、新范围从相反方向进入，以短促翻页过渡明确反馈范围已经变化；翻页提交后在画布顶部居中悬浮显示目标范围相对设备今天的关系，日视图使用“昨天 / 今天 / 明天”“前天 / 后天”及更远的“N 天前 / N 天后”，周、月、年视图对应使用“上 / 本 / 下”和更远的数字差值。在年、月、周、日视图之间切换，或从计时、计划、用户、数据恢复等任意其他页面切换到日历时，也显示同一提示：普通页面切换使用日历保留的当前粒度和锚点，视图切换使用切换后的粒度和锚点，携带计划或记录定位 Handoff 时等待目标粒度、锚点和画布刷新完成后再显示；应用冷启动时日历页的首次显示、日历页自身重复显示或应用从后台恢复但可见页面未改变时不提示。提示使用与记录页“最近记录”中 `new` 一致的 `#9a5550` 粗体大字，持续 3 秒后消失；期间再次翻页、切换视图或重新进入日历时更新文案并重新计时。过渡与提示均不修改日期切换规则或数据。
- 新建计划入口为右下悬浮圆形“+”，表单使用底部弹窗。查看今天或未来日期时，日期预填当前浏览锚点；查看历史日期时，日期回填设备当天。编辑普通计划时复用同一弹窗、字段顺序和时间表单布局，预填该计划现有值，弹窗标题与确认文案切换为“编辑计划 / 保存”；不再保留独立的“编辑计划块”弹窗。编辑态同样显示“固定日程”开关；关闭时保存普通计划修改，开启时按 [3.3 其他核心对象边界](#33-其他核心对象边界) 的原子规则完成转换或并存创建。两条内部路径使用相同表单和成功反馈。
- 编辑固定日程也复用该弹窗，标题为“编辑固定日程”，预填所选实例的标题、显示时间、任务、优先级和当前规则的频率、间隔、星期或日期。该模式始终展开固定日程表单且不显示关闭开关，不能借此转换为普通计划；保存只作用于“本次及后续”，成功提示“固定日程已更新”。编辑后的开始日期不得早于所选实例所在的设备本地自然日；同一自然日内允许调整显示时间。若新规则的生效槽位本身不命中新重复日期，则只刷新画布，不强制产生或定位首项。
- 新建成功后，普通计划以实际写入的 `CalendarEvent` 为定位目标；固定日程以创建结果中可立即投影的首项 `virtual plan` 为定位目标，若规则生效时间本身不命中所选重复日期，则仅刷新当前画布。定位目标不与当前浏览范围相交时，先把浏览锚点切换到目标开始时间所在的同粒度范围；画布刷新后，只有当目标未完整显示在当前可见区域内时，才平滑滚动到附近，已经可见时保持用户当前滚动位置。日视图中，若定位目标原本会进入重叠条目的“+N”集合块，本次定位布局必须让目标保持为独立可见块，并把与目标冲突最少的原可见轨道条目移入集合块；被移入集合的旧条目不得同时保留普通块。完整重叠时只替换最后一条原可见计划，目标跨越同轨多个不相交条目时只替换几何上必要的最少条目。周、月、年视图没有聚合块，定位到目标的第一个可见分段。该保护只作用于创建后的定位刷新，不改变持久化顺序或后续普通刷新的布局规则。计划页长按任务标题跳转已有计划或关联时间记录时，日历按本节定位规则落到日视图对应块，并加上持续 3 秒的 `4rpx` 灰绿描边 `#78947f`；既没有计划候选也没有关联记录时，只有未完成任务可留在计划页创建计划，已完成任务不执行任何操作。完整入口规则见 [4.2 愿望/项目视图](#42-愿望项目视图)。
...
日历和统计查询按实例的最终时间区间判断是否纳入：`endedAt > rangeStart && startedAt <= rangeEnd`，即与查询范围存在非零交集。跨越日、周、月或年范围起点的实例仍应被投影；固定日程不提供单次改期，按“本次及后续”拆分后，旧规则与新规则分别按各自实例的最终区间决定纳入，跳过实例不纳入。纳入后的记录保留完整持续时间，不在查询边界处裁剪。
...
- 页面顶部用固定悬浮的复盘范围选择器替代英文资料库标记。主页签依次为“本周复盘”“月度复盘”“年度复盘”，首次进入默认选择“本周复盘”。选择月度或年度后，在同一选择器内显示最近 5 个周期的次级页签，即当前月/年与此前 4 个月/年；同时提供自定义周期表单，月份使用 `YYYY-MM`、年份使用 `YYYY`，提交后可查看任意有效的当前或历史月/年，禁止选择未来周期。自定义周期不改写最近 5 项。范围选择只在当前页面会话内保留，不进入资料库、本地界面偏好或 JSON 导入导出。
- 本周从设备本地周一开始。当前周、月、年的复盘范围从周期起点截止到当前时刻，过去月、年使用完整自然周期，避免尚未开始且尚无实际记录的未来计划提前形成偏差。日志或计划实例只要与查询范围存在非零交集，就按既有统计规则计入完整持续时间，不在查询边界裁剪；因此跨越当前时刻的进行中计划仍计入完整计划时长。范围内日志关联尚未纳入范围的计划时，按 [3.1.1 计划与实际偏差](#311-计划与实际偏差) 将该计划实例随日志带入。除下述偏差趋势图的历史上下文窗口外，页面全部统计都使用当前选定的复盘范围。
- 默认只统计 `confirmed`，可选择包含 `candidate` 估算。该开关影响投入时间、记录数、标签投入、项目投入、计划的实际时间、偏差明细、偏差趋势和投入饼图；计划数与计划时间只来自计划实例，不受该开关影响。
- 复盘总览不重复显示“本周复盘”等模块标题，使用两行两列固定展示：左上“投入时间（分钟）”、右上“记录数”、左下“计划时间（分钟）”、右下“计划数”。“投入时间”的主数字是所选范围的总实际投入；计划外投入大于零时，在主数字后显示辅助文案 `（含计划外 N）`，其中 `N` 是同一统计范围内的计划外投入分钟数，例如 `356（含计划外 221）`；计划外投入为零时不显示括号。括号使用明显小于主数字的辅助字号并与主数字基线对齐，狭窄屏幕允许整段换行但不得截断。“包含候选估算”同时影响主数字和括号内计划外分钟数。计划数等于按 [3.1.1 计划与实际偏差](#311-计划与实际偏差) 纳入的普通 `CalendarEvent` 与有效、未跳过的重复规则投影实例总数，每个计划块实例只计一次；计划时间等于这些实例完整计划时长之和。已经关联日志的重复实例仍保留其计划数和计划时间。
- 复盘总览内显示“计划与实际偏差”柱状趋势图。每个桶的值是桶内各计划块实例 `实际时间 - 计划时间` 的有符号偏差之和，只汇总与计划块实例有效关联的纳入日志；桶内日志关联尚未纳入该桶的计划实例时，同样按 [3.1.1 计划与实际偏差](#311-计划与实际偏差) 把实例带入该桶。计划外日志仍进入投入时间、记录数和标签汇总，但不进入偏差趋势。正值表示实际超出计划，负值表示实际少于计划，零轴和正负方向不得只依赖颜色区分。
  - 本周复盘最多显示截止当前周的 8 个设备本地自然周，最后一周截止当前时刻。
  - 月度复盘最多显示 8 个设备本地自然周：过去月份截止到包含该月最后一天的完整自然周，本月截止当前周的当前时刻。完整周可以带入所选月份之前或之后的少量日期，这是偏差趋势相对复盘主范围的明确例外。
  - 年度复盘最多显示所选年内截止锚点的 6 个自然月：过去年度截止十二月，本年度截止当前月且最后一月截止当前时刻；不足 6 个月时不跨到所选年的前一年补齐。
  - 趋势从最早包含可比较计划实例的桶开始，最多保留上述桶数；起止桶之间没有计划实例的周期仍显示零值。整个窗口没有可比较计划实例时，不渲染趋势标题、正负值说明、图表或空状态文案，即隐藏整个趋势区块。
- 标签投入、项目投入和计划与实际偏差明细均先排序再展示。标签和项目按投入时间从长到短排列；偏差按 `abs(实际时间 - 计划时间)` 从大到小排列，使超出或不足最多的计划优先，绝对值相同时依次按计划开始时间和稳定计划实例 ID 排序。计划外日志不作为偏差明细条目。
- 上述三个明细模块默认最多显示排序后的前 5 行；总条目超过 5 行时，在标题行最右侧显示“查看更多”文字按钮，点击后用可纵向滚动的底部弹窗展示相同排序下的全部条目。弹窗只扩展列表，不复制饼图；弹窗与卡片中的同一条目共享当前页面状态。
- 标签汇总直接使用日志的 `tags`：每条日志的完整时长分别计入每个唯一标签桶，大小写不同的标签分别统计；空数组通过独立内部哨兵计入界面显示为“无标签”的派生桶，用户创建的字面标签“无标签”仍是另一个普通标签桶。项目汇总必须由日志关联的具体计划块或重复投影实例取得任务，再读取该任务当前有效的 `projectId`；无计划块、计划块缺少有效任务或任务未关联项目的日志不归入具体项目，并在用户视图的项目汇总中进入派生的“未归属项目”桶。任务/项目名称快照只用于历史展示，不参与当前项目统计。
- 标签投入和项目投入按各自条目生成饼图，选择只影响饼图，不改变列表数值、排序、其他模块或任何持久化数据。标签和项目分别维护勾选集合；卡片与“查看更多”弹窗在条目标题左侧显示的同一勾选项实时同步。勾选状态按条目稳定 ID 在当前页面会话内跨复盘范围和“包含候选估算”切换继续生效，新出现条目默认勾选；状态不进入本地界面偏好或 JSON 导入导出。
  - 模块总条目少于 3 个时，不显示勾选框、饼图或选择提示。
  - 模块总条目恰好为 3 个时，不显示勾选框，三项全部计入并直接显示饼图。
  - 模块总条目大于 3 个时显示勾选框。勾选少于 3 项时不绘制饼图，并显示“请至少勾选三项”；勾选至少 3 项时按全部勾选条目绘制饼图。
  - 饼图的 `100%` 等于当前全部勾选条目的时长之和。单项时长严格超过该合计 `10%` 时独立成块并在图例标注名称与百分比；占比不超过 `10%` 的勾选项进入灰色尾部切片。尾部只有一个条目时，切片和图例使用该条目的名称，不显示为“其他”；尾部包含两个及以上条目时才合并并显示为“其他”。独立条目理论上最多为 9 个，使用 9 种普通颜色按当前饼图切片顺序依次分配，同一张图内不得重复；灰色尾部切片始终使用不占用普通色序列的独立灰色。标签维度允许同一日志完整计入多个标签桶，因此这里的百分比分母是所选标签桶合计，不等同于复盘总投入时间。
- 饼图及其绘制层必须服从页面正常层级；页面滚动时，饼图不得越过或遮挡顶部悬浮的复盘范围选择器，也不得因此改变选择器的点击、输入和横向滚动能力；“查看更多”弹窗打开时也不得越过弹窗或遮罩。
- 复盘范围选择器、总览、图表、折叠列表和底部弹窗遵守现有安静、低饱和、轻手账视觉基线；绿色只表达当前选择和实际投入，计划继续使用灰蓝语义。页面原有本地数据风险提示、数据管理区、容量出口及其安全语义保持不变，不受复盘范围选择影响。
...
- 唯一 revision 必须保存有效 `taskId` 与任务名称快照，不保存有效 `projectId`；项目仅在读取时由 `Task.projectId` 派生。规则实体不原地修改标题、时间、频率、间隔、重复日期、任务或优先级；编辑未来范围时按下述规则原子截断旧规则并创建新规则。
- `occurrenceStart` 是固定规则生成的逻辑发生槽位；它是跳过和删除边界的唯一依据，不因界面显示时间或计时实际开始时间改变。
- 未操作实例只存在于视图投影中，即 `virtual plan`，不是 `candidate` 日志。当前活动计时已关联该实例时，不得再确认完成该实例；应先结束计时。计划页计时器 icon 的长按确认完成与日历「确认完成」共用同一写入规则，并遵守 [3.1.2](#312-任务计划实例与完成状态) 的开始时间门槛；尚未记录的普通计划块则按计划起止时间生成 `manual` 来源的 `confirmed` 日志。
- 确认实例时创建日志并保存来源规则与实例标识；跳过实例时保存 `OccurrenceException(kind = 'skip')`，避免下次查看时重新出现。当前 v1 不存在其他例外类型。
- “编辑本次及后续”的边界必须是旧规则可投影且未跳过的真实 `occurrenceStart`。保存前和事务内均检查边界起的未来范围：只要任意 `TimeLog`（无论 `candidate` 或 `confirmed`）、活动计时草稿或恢复草稿通过旧 `originRuleId + originOccurrenceId` 关联到边界或后续实例，就禁止编辑；禁止时不截断旧规则、不创建新规则，也不清理 skip。页面只把检查结果用于隐藏入口，不能代替事务校验。
- 编辑通过后，边界前存在逻辑实例时把旧规则唯一 revision 的 `effectiveUntil` 设为 `occurrenceStart - 1`；否则删除旧规则。随后以表单值创建一个新 `ruleId`、单 revision 规则承接未来，旧规则的标题、任务、时间和重复模式继续解释过去实例。新规则的 `effectiveFrom` 使用编辑后的逻辑槽位，且其设备本地自然日不得早于旧边界所在自然日；不要求该槽位命中新模式。编辑不提供“仅此一次”，不修改或重绑过去的日志、计时及实例标识。
- 编辑边界前的 skip 例外保留；旧规则边界及后续的 skip 例外直接清除，不迁移到新 `ruleId`，也不向用户额外提示。旧规则截断或删除、新规则创建、skip 清理和旧/新任务状态重算必须在同一个 `LocalRepository.transaction` 中完成；任一步校验或写入失败时完整回滚。
...
- 基础项目和标签统计（含派生的“无标签”桶）、计划与实际偏差，以及本周、月度和年度复盘。
